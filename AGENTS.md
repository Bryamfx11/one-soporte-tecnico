...
- `JWT_SECRET` is **required in production** (`server/auth.js` throws); tokens are HS256 with issuer `one-soporte` verified on every request
- Hardening: the central error handler in `app.js` classifies body-parser failures (malformed JSON → 400, oversized body → 413) and never exposes internals; API responses carry `Cache-Control: no-store`; the production request log **redacts** the portal `clave` query param; `security.js` also sets `Cross-Origin-Opener-Policy`, `Cross-Origin-Resource-Policy` and `Permissions-Policy`; adjunto base64 is validated as a linear char check (a canonical `(?:...)*(...)?` regex **overflows the stack on large strings** — do not reintroduce)
...
- `server/auth.js` `requireAuth` selects `activo` from `usuarios`; returns 401 when `activo === 0` (login also rejects deactivated users). `GET /api/usuarios`, `POST /api/auth/register` (creates accounts with `rol` `admin`/`tecnico`, 409 on duplicate email) and `PATCH /api/usuarios/:id` require admin — the PATCH edits `nombre`/`email`/`rol`, resets `password` and toggles `activo`; it blocks self-deactivation, self-role change and removing the last admin (409 on duplicate email, 400 with `details` for validation)
