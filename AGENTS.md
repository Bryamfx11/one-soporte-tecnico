...
- `/api/sse/events` (SSE, `requireAuth`) pushes `{"type":"update"}` whenever incidents/técnicos change; client subscribes via `useLiveData(reload, { onChange })` in `client/src/sse.js` (Dashboard + Indicadores, no polling). `onChange` fires after each `update` (used to show "Actualizado HH:MM")
