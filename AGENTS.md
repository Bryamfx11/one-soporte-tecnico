...
- `server/auth.js` `requireAuth` selects `activo` from `usuarios`; returns 401 when `activo === 0` (login also rejects deactivated users). `GET/PATCH /api/usuarios` and `POST /api/auth/register` (creates accounts with `rol` `admin`/`tecnico`, 409 on duplicate email) require admin
